<template>
    <div
        ...
        <div>
            <h3 data-test="recipe-name" 
                class="text-lg font-bold text-blue-900" 
                v-if="recipe.name">{{ recipe.name }}</h3>
            <p data-test="recipe-description" 
                class="text-sm mt-2 text-gray-700 dark:text-gray-300" 
                v-if="recipe.description">{{ truncate(recipe.description) }}</p>
        </div>
